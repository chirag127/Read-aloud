...
class SettingsPanel {
  ...
  create(voices) {
    ...
    this.panel.innerHTML = `
      ...
      <input type="range" id="read-aloud-speed" min="0.5" max="2" step="0.1" value="${this.settings.speed}">
      <span id="read-aloud-speed-value">${this.settings.speed.toFixed(1)}</span>
      
      <label for="read-aloud-pitch">Pitch</label>
      <input type="range" id="read-aloud-pitch" min="0.5" max="2" step="0.1" value="${this.settings.pitch}">
      ...
      <select id="read-aloud-voice">
        ...
        ${this.voices.map(voice => `
          <option value="${voice.name}" ${this.settings.voice === voice.name ? 'selected' : ''}>
   ...
   */
  saveSettings() {
    StorageManager.saveSettings(this.settings).then(() => {
  ...
  async updateSettings() {
    try {
      ...
      if (speedValue) speedValue.textContent = this.settings.speed.toFixed(1);
